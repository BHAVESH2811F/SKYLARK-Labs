...
# Create the dataloader
        train_loader = torch.utils.data.DataLoader(dataset, batch_size=32, drop_last=False, shuffle=True)
    ...
    def update(self, batch_original, batch_augmented):
        ...
        predictions_online_original = self.predictor(self.online_network(batch_original))
        # compute key features
        with torch.no_grad():
            targets_augmented = self.target_network(batch_augmented)
        loss = self.regression_loss(predictions_online_original, targets_augmented)
        del predictions_online_original, targets_augmented
        #torch.cuda.empty_cache()
        
        predictions_online_augmented = self.predictor(self.online_network(batch_augmented))
        with torch.no_grad():
            targets_original = self.target_network(batch_original)
        
        loss += self.regression_loss(predictions_online_augmented, targets_original)
        
        #Free memory

        del predictions_online_augmented, targets_original
        torch.cuda.empty_cache()
